...
stat_by_teachers = pd.DataFrame([
    {'title': 'РФ', 'value': df_teachers['salary'].mean() / 1000},
    {'title': region, 'value': df_teachers[df_teachers['region'] == region]['salary'].mean() / 1000},
    {'title': university, 'value': df_teachers[df_teachers['university_abbreviation'] == university]['salary'].mean() / 1000},
]).dropna()
fig, ax = plt.subplots()
...
description_top_by_salary = texts['top_by_salary'].format(
    '; '.join([f"{i} ({x['id']})" for i, x in df_top_by_salary[['region', 'id']].groupby(by='region').count().sort_values(by='id', ascending=False).iterrows()]),
    len(df_top_by_salary['professional_roles'].unique()),
    '; '.join([f"{i} ({x['id']})" for i, x in df_top_by_salary[['professional_roles', 'id']].groupby(by='professional_roles').count().sort_values(by='id', ascending=False).iterrows()])
)
